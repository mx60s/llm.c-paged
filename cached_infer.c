...
void attention_forward(float* out, float* preatt, float* att,
                       float* inp_q, float* inp_k, float* inp_v,
                       int B, int T, int C, int NH) {
    // inp_q, inp_k, inp_v are (B, T, C) for query, key, value vectors respectively
    ...
    for (int b = 0; b < B; b++) {
        for (int t = 0; t < T; t++) {
            for (int h = 0; h < NH; h++) {
                float* query_t = inp_q + b * T * C + t * C + h * hs;
                float* preatt_bth = preatt + b * NH * T * T + h * T * T + t * T;
                float* att_bth = att + b * NH * T * T + h * T * T + t * T;

                // pass 1: calculate query dot key and maxval
                float maxval = -10000.0f; // large negative for numerical stability
                for (int t2 = 0; t2 <= t; t2++) {
                    float* key_t2 = inp_k + b * T * C + t2 * C + h * hs;
                ...
                for (int t2 = 0; t2 <= t; t2++) {
                    float* value_t2 = inp_v + b * T * C + t2 * C + h * hs;
...
#define NUM_PARAMETER_TENSORS 20
typedef struct {
    float* wte; // (V, C)
    float* wpe; // (maxT, C)
    float* ln1w; // (L, C)
...
float* malloc_and_point_parameters(ParameterTensors* params, size_t* param_sizes) {
    ...
    float** ptrs[] = {
        &params->wte, &params->wpe, &params->ln1w, &params->ln1b, &params->qw, &params->kw, &params->vw, &params->qb, &params->kb, &params->vb,
...
typedef struct {
    ...
    //float* qkv; // (L, B, T, 3*C)
    float* q; // (L, B, T, C)
    float* k; // (L, B, max_len, C)
    float* v; // (L, B, max_len, C)
    float* atty; // (L, B, T, C)
...
float* malloc_and_point_activations(ActivationTensors* acts, size_t* act_sizes) {
    ...
    float** ptrs[] = {
        &acts->encoded, &acts->ln1, &acts->ln1_mean, &acts->ln1_rstd, &acts->q, &acts->k, &acts->v, &acts->atty,
...
void gpt2_build_from_checkpoint(GPT2 *model, const char* checkpoint_path) {
    ...
    model->param_sizes[4] = L * C * C; // qw
    model->param_sizes[5] = L * C * C; // kw
    model->param_sizes[6] = L * C * C; // vw
    model->param_sizes[7] = L * C; // qb
    model->param_sizes[8] = L * C; // kb
    model->param_sizes[9] = L * C; // vb
    model->param_sizes[10] = L * C * C; // attprojw
    model->param_sizes[11] = L * C; // attprojb
    model->param_sizes[12] = L * C; // ln2w
    model->param_sizes[13] = L * C; // ln2b
    model->param_sizes[14] = L * (4 * C) * C; // fcw
    model->param_sizes[15] = L * (4 * C); // fcb
    model->param_sizes[16] = L * C * (4 * C); // fcprojw
    model->param_sizes[17] = L * C; // fcprojb
    model->param_sizes[18] = C; // lnfw
    model->param_sizes[19] = C; // lnfb
...
void gpt2_forward(GPT2 *model, int* inputs, int* targets, size_t B, size_t T, size_t max_total) {
    ...
    if(model->acts_memory == NULL) {
        use_kv_cache = 0;
        printf("Allocating space for activations\n");
        ...
        model->act_sizes[4] = L * B * T * C; // q
        model->act_sizes[5] = L * B * max_total * C; // k
        model->act_sizes[6] = L * B * max_total * C; // v
        model->act_sizes[7] = L * B * T * C;  // atty
        model->act_sizes[8] = L * B * NH * T * T;  // preatt
        model->act_sizes[9] = L * B * NH * T * T;  // att
        model->act_sizes[10] = L * B * T * C; // attproj
        model->act_sizes[11] = L * B * T * C; // residual2
        model->act_sizes[12] = L * B * T * C; // ln2
        model->act_sizes[13] = L * B * T; // ln2_mean
        model->act_sizes[14] = L * B * T; // ln2_rstd
        model->act_sizes[15] = L * B * T * 4*C; // fch
        model->act_sizes[16] = L * B * T * 4*C; // fch_gelu
        model->act_sizes[17] = L * B * T * C; // fcproj
        model->act_sizes[18] = L * B * T * C; // residual3
        model->act_sizes[19] = B * T * C; // lnf
        model->act_sizes[20] = B * T; // lnf_mean
        model->act_sizes[21] = B * T; // lnf_rstd
        model->act_sizes[22] = B * T * V; // logits
        model->act_sizes[23] = B * T * V; // probs
        model->act_sizes[24] = B * T; // losses
        size_t num_activations = 0;
        for (size_t i = 0; i < NUM_ACTIVATION_TENSORS; i++) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        float* l_qw = params.qw + l * C * C;
        float* l_kw = params.kw + l * C * C;
        float* l_vw = params.vw + l * C * C;
        float* l_qb = params.qb + l * C;
        float* l_kb = params.kb + l * C;
        float* l_vb = params.vb + l * C;
        ...
        float* l_q = acts.q + l * B * T * C;
        float* l_k = acts.k + l * B * max_total * C;
        float* l_v = acts.v + l * B * max_total * C;
        ...
        use_kv_cache = 0;
        if (use_kv_cache)
            // uses the t-1 values from cache and adds the last one
            matmul_forward_qkv_cached(l_q, l_k, l_v, l_ln1, l_qw, l_kw, l_vw, l_qb, l_kb, l_vb, B, T, C, C);
        else
            // generates and stores all k and v values
            matmul_forward_qkv(l_q, l_k, l_v, l_ln1, l_qw, l_kw, l_vw, l_qb, l_kb, l_vb, B, T, C, C);

        attention_forward(l_atty, l_preatt, l_att, l_q, l_k, l_v, B, T, C, NH);
...
int main(int arc, char **argv) {
    ...
    for (int t = PROMPT_SIZE; t < T; t++) {
        gpt2_forward(&model, gen_tokens, NULL, B, T, totalSize);
    ...
    for (int t = T; t < totalSize; t++) {
        gpt2_forward(&model, gen_tokens + (t - T), NULL, B, T, totalSize);
        // printf("finished forward\n");
